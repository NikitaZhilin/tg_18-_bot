...
| Самодостаточный сценарий каждой карточки | Готово | 218 карточек со структурой по типу и конкретными действиями |
| Отключенные пользовательские шаблоны сложных практик | Готово | 3 `draft`-карточки «Экстрима», исключенные из рулетки |
| Мягкие манжеты как отдельный нерулеточный реквизит | Готово | миграция `007_soft_cuffs_item.sql`, закрытая карточка |
...
Результат последней проверки: 64 из 64 тестов прошли.
...
- Карточки «Экстрима» требуют пароля раздела и подходящего обязательного реквизита; обычные переключатели level 4/hard не дублируются.
